...
type ServerConfig struct {
	...
	BehindCloudflare bool     `mapstructure:"behind_cloudflare"`
...
// FloodControlConfig groups rate limiting and brute-force protection settings.
// Set enabled: false to disable all flood control.
type FloodControlConfig struct {
	Enabled              bool `mapstructure:"enabled"`
	HTTPRateLimitPerMin  int  `mapstructure:"http_rate_limit_per_min"`
	QueryRateLimitPerMin int  `mapstructure:"query_rate_limit_per_min"`
	BruteForceMax        int  `mapstructure:"brute_force_max"`
	BruteForceBanMin     int  `mapstructure:"brute_force_ban_min"`
}
...
type BGPConfig struct {
	...
	// AddPathReceive negotiates BGP ADD-PATH receive so multiple paths per prefix
	// can be learned from peers that advertise them.
	AddPathReceive bool `mapstructure:"add_path_receive"`
}
