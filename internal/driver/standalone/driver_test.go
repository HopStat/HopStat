...
func TestDriverCapabilities(t *testing.T) {
	cmds := []domain.CommandType{domain.CmdPing, domain.CmdTraceroute, domain.CmdBGPRoute}
...
func TestDriverPingInvalidTarget(t *testing.T) {
	...
	_, err := drv.Ping(context.Background(), "127.0.0.1", 3)
	if err != domain.ErrInvalidTarget {
		t.Errorf("expected ErrInvalidTarget, got %v", err)
	}
}

func TestDriverPingDNSNotFound(t *testing.T) {
	node := &domain.Node{EnabledCmds: []domain.CommandType{domain.CmdPing}}
	drv, _ := NewDriver(node, nil)

	_, err := drv.Ping(context.Background(), "hopstat-invalid-host.example", 3)
	if err != domain.ErrDNSNotFound {
		t.Errorf("expected ErrDNSNotFound, got %v", err)
	}
}
...
func TestDriverBGPRouteInvalidPrefix(t *testing.T) {
	...
	_, err := drv.BGPRoute(context.Background(), "not-a-prefix/24")
	if err != domain.ErrInvalidTarget {
		t.Errorf("expected ErrInvalidTarget, got %v", err)
	}
}

func TestDriverBGPRouteDNSNotFound(t *testing.T) {
	node := &domain.Node{EnabledCmds: []domain.CommandType{domain.CmdBGPRoute}}
	drv, _ := NewDriver(node, nil)

	_, err := drv.BGPRoute(context.Background(), "hopstat-invalid-host.example")
	if err != domain.ErrDNSNotFound {
		t.Errorf("expected ErrDNSNotFound, got %v", err)
	}
}
