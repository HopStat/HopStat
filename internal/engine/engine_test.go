...
func TestExecuteNodeNotFound(t *testing.T) {
	...
	e := New(&QueryConfig{MaxConcurrent: 10}, repo, nil, nil, nil, nil, 0)
...
func TestClassifyError(t *testing.T) {
	tests := []struct {
		...
		{domain.ErrDNSNotFound, "DNS_NOT_FOUND"},
...
func TestNewEngine(t *testing.T) {
	...
	e := New(cfg, &mockNodeRepo{}, nil, nil, nil, nil, 0)
	if e == nil {
...
func TestEngineEnrichHopsNilGeo(t *testing.T) {
	e := New(&QueryConfig{MaxConcurrent: 10}, &mockNodeRepo{}, nil, nil, nil, nil, 0)
...
func TestEngineEnrichASPathNilGeo(t *testing.T) {
	...
	br := &domain.BGPResult{
		Routes: []domain.BGPRoute{
			{Prefix: "1.1.1.0/24", ASPath: []uint32{13335, 15169}},
		},
	}
	result := &domain.QueryResult{}
	e.applyBGPASPath(context.Background(), br, "1.1.1.1", result, ExecuteOption{})
	if len(result.ASPath) != 2 || result.ASPath[0] != 13335 {
		t.Fatalf("expected AS path on result, got %v", result.ASPath)
	}
	if len(result.ASPathEnriched) != 0 {
		t.Error("expected no enrichment with nil geoDB")
	}
}

func TestClassifyAndSanitizeErrors(t *testing.T) {
	cases := []struct {
		err      error
		code     string
		sanitize string
	...
	for _, tc := range cases {
		if got := ClassifyError(tc.err); got != tc.code {
			t.Fatalf("%v: code = %q want %q", tc.err, got, tc.code)
		}
		if got := SanitizeErrorMsg(tc.err); got != tc.sanitize {
			t.Fatalf("%v: msg = %q want %q", tc.err, got, tc.sanitize)
		}
	}
}

func BenchmarkExecuteValidationPath(b *testing.B) {
	repo := &mockNodeRepo{
		node: &domain.Node{
			ID:          1,
	...
	e := New(&QueryConfig{MaxConcurrent: 64}, repo, nil, nil, nil, nil, 0)
	q := &domain.Query{
		ID:       "bench",
		NodeID:   1,
		Command:  domain.CmdPing,
