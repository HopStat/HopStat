...
func setupTestDB(t *testing.T) (*sql.DB, *Queries) {
	...
	_, _ = db.Exec(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT NOT NULL UNIQUE, password_hash TEXT NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, last_login DATETIME)`)
	_, _ = db.Exec(`CREATE TABLE IF NOT EXISTS nodes (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', type TEXT NOT NULL DEFAULT 'standalone', city TEXT NOT NULL DEFAULT '', country TEXT NOT NULL DEFAULT '', lat REAL, lon REAL, credential_id INTEGER, active INTEGER NOT NULL DEFAULT 1, is_default INTEGER NOT NULL DEFAULT 0, enabled_cmds TEXT NOT NULL DEFAULT '[]', bgp_config TEXT, agent_url TEXT NOT NULL DEFAULT '', agent_token TEXT NOT NULL DEFAULT '', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP)`)
...
func TestQueries_GetUserByEmail(t *testing.T) {
	...
	_, err := db.ExecContext(ctx, `INSERT INTO users (email, password_hash) VALUES (?, ?)`,
		"admin@example.com", "$2a$10$hashhash")
	if err != nil {
	...
	if fetched == nil || fetched.Email != "admin@example.com" {
		t.Fatalf("unexpected user: %+v", fetched)
	}

	notFound, err := q.GetUserByEmail(ctx, "nobody@example.com")
	if err != nil || notFound != nil {
		t.Fatalf("expected nil for missing user, got %v err %v", notFound, err)
	}
}
...
func TestQueries_AuditLog_CRUD(t *testing.T) {
	...
	userRes, _ := db.ExecContext(ctx, `INSERT INTO users (email, password_hash) VALUES (?, ?)`,
		"audituser@example.com", "hash")
	...
	if logs[0].NodeName != "audit-node" {
		t.Errorf("node_name: got %q, want %q", logs[0].NodeName, "audit-node")
	}
	...
	todayCount, err := q.CountAuditLogsToday(ctx)
	if err != nil {
		t.Fatalf("CountAuditLogsToday: %v", err)
	}
	if todayCount != 2 {
		t.Errorf("today count: got %d, want 2", todayCount)
	}
