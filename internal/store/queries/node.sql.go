...
const nodeCols = `id, name, description, type, city, country, lat, lon, credential_id, active, is_default, enabled_cmds, bgp_config, agent_url, agent_token, created_at, updated_at`
...
func scanNode(scanner interface{ Scan(...interface{}) error }, n *Node) error {
	return scanner.Scan(&n.ID, &n.Name, &n.Description, &n.Type, &n.City, &n.Country,
		&n.Lat, &n.Lon, &n.CredentialID, &n.Active, &n.IsDefault, &n.EnabledCmds, &n.BGPConfig,
...
func (q *Queries) GetActiveNodes(ctx context.Context) ([]Node, error) {
	rows, err := q.db.QueryContext(ctx, `SELECT `+nodeCols+` FROM nodes WHERE active = 1 ORDER BY is_default DESC, name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) CreateNode(ctx context.Context, arg *Node) (*Node, error) {
	result, err := q.db.ExecContext(ctx, `
		INSERT INTO nodes (name, description, type, city, country, lat, lon, credential_id, active, is_default, enabled_cmds,
	...
	`, arg.Name, arg.Description, arg.Type, arg.City, arg.Country, arg.Lat, arg.Lon, arg.CredentialID,
		arg.Active, arg.IsDefault, arg.EnabledCmds, arg.BGPConfig, arg.AgentURL, arg.AgentToken)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) UpdateNode(ctx context.Context, arg *Node) (*Node, error) {
	_, err := q.db.ExecContext(ctx, `
		UPDATE nodes SET name = ?, description = ?, type = ?, city = ?, country = ?, lat = ?, lon = ?,
		                 credential_id = ?, active = ?, is_default = ?, enabled_cmds = ?, bgp_config = ?, agent_url = ?,
		                 agent_token = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`, arg.Name, arg.Description, arg.Type, arg.City, arg.Country, arg.Lat, arg.Lon,
		arg.CredentialID, arg.Active, arg.IsDefault, arg.EnabledCmds, arg.BGPConfig, arg.AgentURL, arg.AgentToken, arg.ID)
	if err != nil {
		return nil, err
	}
...
func (q *Queries) SetDefaultNode(ctx context.Context, id int64) error {
	...
	res, err := tx.ExecContext(ctx, `UPDATE nodes SET is_default = 1 WHERE id = ?`, id)
	if err != nil {
		return err
	}
