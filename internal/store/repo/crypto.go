...
var (
	readRandom   = func(r io.Reader, b []byte) (int, error) { return io.ReadFull(r, b) }
	newAESCipher = aes.NewCipher
	newAESGCM    = cipher.NewGCM
)
...
func Encrypt(plaintext, hexKey string) (string, error) {
	...
	aesGCM, err := newAESGCM(block)
	if err != nil {
		return "", fmt.Errorf("create GCM: %w", err)
	}

	nonce := make([]byte, aesGCM.NonceSize())
	if _, err := readRandom(rand.Reader, nonce); err != nil {
