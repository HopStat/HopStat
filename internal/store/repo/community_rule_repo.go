...
func (r *communityRuleRepo) GetActive(ctx context.Context) ([]*domain.CommunityRule, error) {
	rules, err := r.q.GetActiveCommunityRules(ctx)
	if err != nil {
		return nil, err
	}
	result := make([]*domain.CommunityRule, len(rules))
...
func mapCommunityRule(r *queries.CommunityRule) *domain.CommunityRule {
	rule := &domain.CommunityRule{
		...
		Severity:    domain.NormalizeSeverity(domain.Severity(r.Severity)),
