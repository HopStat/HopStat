...
var readTranslationFile = os.ReadFile
...
func LoadTranslations(dir string) error {
	...
	for _, entry := range entries {
		...
		data, err := readTranslationFile(dir + "/" + entry.Name())
		if err != nil {
			continue
		}
...
var splitAcceptLanguage = strings.Split

func DetectLanguage(acceptLanguage string) string {
	if acceptLanguage == "" {
		return defaultLang
	}
	parts := splitAcceptLanguage(acceptLanguage, ",")
	if len(parts) > 0 {
