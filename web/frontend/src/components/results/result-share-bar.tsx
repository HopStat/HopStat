...
export function ResultShareBar({ summary, shareUrl, buildText }: Props) {
  const { t } = useI18n()
  const [copied, setCopied] = useState<'link' | 'result' | null>(null)

  async function copy(kind: 'link' | 'result', text: string) {
    try {
  ...
  return (
    <div className="result-share-bar flex min-w-0 items-center gap-2 px-1">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1">
        ...
        <button
          type="button"
          onClick={() => void copy('result', buildText())}
          className={buttonClass}
        >
          {copied === 'result' ? (
            <><Check className="w-3 h-3 text-brand" aria-hidden /><span>{t('result.copied')}</span></>
          ) : (
            <><Copy className="w-3 h-3" aria-hidden /><span>{t('result.copy_result')}</span></>
          )}
        </button>
      </div>
    </div>
